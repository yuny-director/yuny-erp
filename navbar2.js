// 🚨 YUNY_ERP 전역 네비게이션바 (정식 원가 품목 미매핑 노출 제외 보완 완본)
(function() {
    ...
    function initNavbar() {
        ...
        <style>
            ...
            .navbar-user-name-text { font-size: 13px; font-weight: bold; color: #ffffff; margin: 0 4px; }
            ...
            .account-modal-card { background-color: #ffffff; padding: 28px; border-radius: 16px; width: 95%; max-width: 1400px; box-shadow: 0 20px 40px rgba(0,0,0,0.25); box-sizing: border-box; border: 1px solid #e2e8f0; animation: modalFadeIn 0.25s ease-out; }
            ...
            .account-form-container { display: flex; align-items: flex-end; gap: 12px; background-color: #f8fafc; padding: 16px; border-radius: 12px; border: 1px solid #e2e8f0; box-sizing: border-box; }
            .acc-form-item { flex: 1; display: flex; flex-direction: column; gap: 6px; }
            .acc-form-item label { font-size: 11px; font-weight: 700; color: #475569; margin: 0; display: block; white-space: nowrap; }
            .acc-form-item input, .acc-form-item select { width: 100%; height: 36px; font-size: 12px; padding: 0 10px; border-radius: 6px; border: 1px solid #cbd5e1; background-color: #ffffff; box-sizing: border-box; margin: 0; }
            .acc-form-item input:focus, .acc-form-item select:focus { border-color: #3b82f6; outline: none; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.15); }
            .btn-acc-save { height: 36px; padding: 0 16px; background-color: #2c3e50; color: #ffffff; border: none; border-radius: 6px; font-size: 12px; font-weight: 700; cursor: pointer; white-space: nowrap; transition: background 0.15s; }
            .btn-acc-save:hover { background-color: #1e293b; }
        ...
        <div id="globalMappingModal" class="account-modal-overlay">
            <div class="account-modal-card">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px; border-bottom:2px solid #2c3e50; padding-bottom:10px;">
                    <h3 style="margin:0; color:#2c3e50; font-size:17px;">⚙️ [통합 상품 매핑 센터] 매출·재고 수집 옵션명 ➔ 원가 품목 1:1 수동 매핑</h3>
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                    ...
                    <button class="btn-nav-action btn-nav-purple" style="height:32px; padding:0 14px; font-size:13px;" onclick="window.saveGlobalMappingChanges()">💾 수동 매핑 저장 & 전체 동기화</button>
                </div>

                <div id="viewMapSubUnmapped" style="max-height: 550px; overflow-y: auto; border:1px solid #cbd5e1; border-radius:6px; background:#f8fafc;">
                    <table style="width:100%; border-collapse:collapse; font-size:12px; table-layout:fixed;">
                        <thead style="background:#e2e8f0; position:sticky; top:0; z-index:10;">
                            <tr>
                                <th style="padding:10px; border-bottom:1px solid #cbd5e1; text-align:left;">쇼핑몰 / 엑셀 수집 옵션명</th>
                                <th style="padding:10px; border-bottom:1px solid #cbd5e1; width:300px;">연결할 원가 품목 선택</th>
                                <th style="padding:10px; border-bottom:1px solid #cbd5e1; width:200px;">모델명 선택</th>
                            </tr>
                        </thead>
                ...
                <div id="viewMapSubMapped" style="display:none; max-height: 550px; overflow-y: auto; border:1px solid #cbd5e1; border-radius:6px; background:#ffffff;">
                    <table style="width:100%; border-collapse:collapse; font-size:12px; table-layout:fixed;">
                        <thead style="background:#f1f5f9; position:sticky; top:0; z-index:10;">
                            <tr>
                                <th style="padding:10px; border-bottom:1px solid #cbd5e1; text-align:left;">수집 옵션명</th>
                                <th style="padding:10px; border-bottom:1px solid #cbd5e1; width:300px;">연동된 원가 품목명</th>
                                <th style="padding:10px; border-bottom:1px solid #cbd5e1; width:200px;">모델명</th>
                                <th style="padding:10px; border-bottom:1px solid #cbd5e1; width:80px;">관리</th>
                            </tr>
                        </thead>
        ...
        <div id="accountModal" class="account-modal-overlay">
            <div class="account-modal-card" style="max-width: 880px;">
                ...
                <div class="account-modal-body">
                    <form id="accountForm" onsubmit="return false;" autocomplete="off">
                        <div class="account-form-container">
                            <div class="acc-form-item"><label>ID</label><input type="text" id="accInputId" placeholder="아이디"></div>
                            <div class="acc-form-item"><label>비밀번호</label><input type="password" id="accInputPw" placeholder="비밀번호"></div>
                            <div class="acc-form-item"><label>성함</label><input type="text" id="accInputName" placeholder="성함"></div>
                    ...
                    <div style="max-height: 230px; overflow-y: auto;">
                        <table class="account-table">
                            <thead><tr><th>ID</th><th>성함</th><th>등급</th><th>메뉴 세부 권한 현황</th><th>관리 기능</th></tr></thead>
...
// 🎯 [핵심 보완] 원가관리 정식 품목명에 이미 등록되어 있는 이름은 미매핑 리스트 노출에서 제외
window.renderGlobalMappingTables = function() {
    var unmappedSet = new Set();
    var rawMappings = window.itemMappings || {};

    // 원가관리 정식 품목명 리스트
    var cleanMasterNames = window.stockItems.map(i => (i.itemName || i.name || i.item || "").trim()).filter(Boolean);

    var checkAndAddUnmapped = function(opt) {
        if (!opt) return;
        var cleanOpt = String(opt).trim();
        
        // 🎯 1) 매핑원장에 없고 2) 원가관리 정식 품목명에도 없는 '진짜 미매핑 수집명'만 목록에 표시!
        if (!rawMappings[cleanOpt] && !cleanMasterNames.includes(cleanOpt)) {
            unmappedSet.add(cleanOpt);
        }
    };

    (window.salesRawEntries || []).forEach(e => checkAndAddUnmapped(e.rawOptionName || e.r));
    (window.stockEntries || []).forEach(e => checkAndAddUnmapped(e.rawOptionName || e.item || e.itemName));
    (window.lastUploadedRawOptions || []).forEach(opt => checkAndAddUnmapped(opt));

    var unmappedList = Array.from(unmappedSet);
    document.getElementById('globalUnmappedBadge').innerText = unmappedList.length;
    document.getElementById('globalMappedBadge').innerText = Object.keys(rawMappings).length;

    var uTbody = document.getElementById('globalUnmappedTbody');
    uTbody.innerHTML = "";
    if (unmappedList.length === 0) {
        uTbody.innerHTML = `<tr><td colspan="3" style="padding:20px; color:#27ae60; font-weight:bold; text-align:center;">🎉 미매핑된 수집 옵션이 없습니다! 모두 매핑 완료되었습니다.</td></tr>`;
    } else {
        unmappedList.forEach((rawOpt, idx) => {
            var tr = document.createElement('tr');
            tr.setAttribute('data-key', rawOpt);
            tr.innerHTML = `
                <td class="raw-option-td" style="border-bottom:1px solid #e2e8f0; font-weight:bold; text-align:left; color:#1e293b;">${rawOpt}</td>
                <td style="padding:8px; border-bottom:1px solid #e2e8f0; vertical-align:middle;">
                    <select id="gUnmapItem_${idx}" style="width:100%; height:34px; font-size:12px; font-weight:bold; border-color:#94a3b8;" onchange="window.onGlobalItemChange(this, 'gUnmapModel_${idx}')">
                        ${window.getGlobalItemOptionsHtml("")}
                    </select>
                </td>
                <td style="padding:8px; border-bottom:1px solid #e2e8f0; vertical-align:middle;">
                    <select id="gUnmapModel_${idx}" style="width:100%; height:34px; font-size:12px;">
    ...
    mTbody.innerHTML = "";
    var keys = Object.keys(rawMappings);
    if (keys.length === 0) {
        mTbody.innerHTML = `<tr><td colspan="4" style="padding:20px; color:#94a3b8; text-align:center;">등록된 매핑 정보가 없습니다.</td></tr>`;
    } else {
        keys.forEach((rawOpt, idx) => {
            var mapObj = rawMappings[rawOpt];
            ...
            tr.setAttribute('data-key', rawOpt);
            tr.innerHTML = `
                <td class="raw-option-td" style="border-bottom:1px solid #f1f5f9; font-weight:bold; text-align:left; color:#334155;">${rawOpt}</td>
                <td style="padding:8px; border-bottom:1px solid #f1f5f9; vertical-align:middle;">
                    <select id="gMapItem_${idx}" style="width:100%; height:32px; font-size:12px; font-weight:bold;" onchange="window.onGlobalItemChange(this, 'gMapModel_${idx}')">
                        ${window.getGlobalItemOptionsHtml(curItem)}
                    </select>
                </td>
                <td style="padding:8px; border-bottom:1px solid #f1f5f9; vertical-align:middle;">
                    <select id="gMapModel_${idx}" style="width:100%; height:32px; font-size:12px;">
                        ${window.getGlobalModelOptionsHtml(curItem, curModel)}
                    </select>
                </td>
                <td style="padding:8px; border-bottom:1px solid #f1f5f9; text-align:center; vertical-align:middle;">
                    <button class="btn-nav-action btn-nav-red" style="height:28px; padding:0 8px;" onclick="this.closest('tr').remove()">삭제</button>
                </td>
            `;
...
window.saveGlobalMappingChanges = function() {
    var newMappings = window.itemMappings || {};
    ...
    uRows.forEach((tr, idx) => {
        ...
        if (rawOpt && itemSel && itemSel.value) {
            newMappings[rawOpt.trim()] = { itemName: itemSel.value, modelName: modelSel ? modelSel.value : "" };
        }
    });
    ...
    .then(() => {
        alert("✅ 수동 매핑 정보가 성공적으로 구글 DB에 저장되었습니다! 페이지를 새로고침합니다.");
...
window.autoLoadGlobalUserList = function() {
    ...
    }).catch(e => { console.log("계정 원장 수집 요청 지연중..."); });
};
...
window.saveAccountItem = function() {
    ...
    if (existingIdx > -1) { window.userList[existingIdx] = userObj; } else { window.userList.push(userObj); }
